...
class FunctionalProductSpace(Functional):
    ...
    def _gradient(self, x):
        ...
        gradients = []
        for i in range(self.length):
            gradients.append( self.weights[i] * self.funcs[i](splitted[i]) )
...
'''
from imagingbase.regpy_utils import gradientuniformgrid
from imagingbase.regpy_utils import divergenceuniformgrid

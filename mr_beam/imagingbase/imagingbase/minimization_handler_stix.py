...
class MinimizationHandler:
    def __init__(self, psf_fwhm, npix, fov, obs_sc, prior, zbl, rescaling, data_term, cbar_lims, threshold, repair, to_cat, **kwargs):
        ...
        self.data_fidelity = EhtimFunctional(self.wrapper, self.grid) * self.op
    ...
    def find_initial_guess(self, data, alpha, reverse=True):
        ...
        for j in range(10):
            ...
            for i in range(self.length):
                ...
                self.decision[i, j] = self.data_fidelity(test) + alpha * self.penalty_l0(test)
            guess *= 0
...
#                    self.thresh[index] = test_thresh[index]
        ...
        if reverse:
            for i in range(self.length):
                ...
                for j in range(20):
                    ...
                    test = self.penalty_l0.proximal(init.flatten(), test_thresh)
                    objective = self.data_fidelity(test) + alpha * self.penalty_l0(test)
    ...
    def second_round(self, init, tau, alpha, max_iterations=25, maxiter=20, display=False):
        #Define solver
        solver = Forward_Backward_Splitting(self.setting, self.data_fidelity, self.penalty_l0, init.copy(), tau = tau, regpar = alpha)
        ...
        for i in range(maxiter):
            ...
            print('Data Fidelity: ', self.data_fidelity(solver.x))
    ...
    def third_round(self, init, data_term, tau=10, **kwargs):
        data_fidelity_all = data_term['vis'] * self.data_fidelity

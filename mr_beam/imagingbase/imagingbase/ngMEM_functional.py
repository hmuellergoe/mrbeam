...
class TemporalEntropy(Functional):
    ...
    def _gradient(self, imvecs):
        ...
        for i in range(self.nr_of_frames):
            for j in range(self.nr_of_frames):
                self.imvecdiff[:,i,j] = (_imvecs[i]-_imvecs[j]).flatten()
        signs = np.sign(self.imvecdiff)
        self.imvecdiff = np.abs(self.imvecdiff)
        return -np.sum(np.exp(-self.tdiff**2/(2*self.tau**2))*(signs*np.log(self.imvecdiff+self.C)+signs*self.imvecdiff/(self.imvecdiff+self.C)), axis=1).transpose().flatten()

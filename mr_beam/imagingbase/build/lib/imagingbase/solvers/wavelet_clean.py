...
class DOGCLEAN(Solver):
    def __init__(self, dmap, dbeam, psf, window, widths, add_only_positive_coeff=False, gain=0.1, angle=0, ellipticities=1, num_cores=1, md=False, smoothing_scale=None, weights=None, **args):
        ...
        self.beam_list = np.zeros((self.length, 2*(self.shape[0]//2)+self.shape[0], 2*(self.shape[1]//2)+self.shape[1], self.length)) 
...
class BesselCLEAN(Solver):
    def __init__(self, dmap, dbeam, psf, window, widths, gain=0.1, angle=0, ellipticities=1, num_cores=1, md=False, add_only_positive_coeff=False, support=None, smoothing_scale=None, weights=None, **args):
        self.num_cores = num_cores
